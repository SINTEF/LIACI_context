...
def register_callbacks(app):
    ...
    def get_the_data(filter_options):
        filter_options = FilterOptions(**filter_options)
        print("requesting data for filter", filter_options)

        if len(filter_options.inspections) == 0:
            print("no inspection selected, returning nothing!")
            return {
                'nodes': {},
                'similar_to': [],
                'visually_similar_to': [],
                ...
                'shows_part': [],
                'part_of_ship': [],
                'filter_id': filter_options.filter_id,
                'q_hist': [],
                'd_hist': [],
                'mg_hist': [],
                'sim_hist': [],
        ...
        """
        query1: Images and Inspections

        Get all relevant Image nodes and their Inspections
        Implicitly generate relationships from the node to the inspection
        """
...
# TODO Do this for key frames: https://neo4j.com/docs/graph-data-science/current/algorithms/louvain/
        ...
        all_images_and_inspections = f"""{neighbor_images} WITH ilist + collect(i2) as alli UNWIND alli as i MATCH (i:Image) <-[:HAS_FRAME]- (ins:Inspection)"""

        graph_images = images_and_inspections
        ...
        query2 = f"""{graph_images} WITH i ORDER BY i.id LIMIT {LIMIT} MATCH (i) -[r:SIMILAR_TO]- (i2:Image) WHERE r.distance < {filter_options.telemetry_similarities} RETURN i, i2, r.distance as d"""
        query3 = f"""{graph_images} WITH i ORDER BY i.id LIMIT {LIMIT} MATCH (i) -[r:VISUALLY_SIMILAR_TO]- (i2:Image) WHERE r.distance < {filter_options.visual_similarities} RETURN i, i2, r.distance as d"""


        """
        query4: find mosaics
        """
        query4 = f"""{graph_images} WITH i ORDER BY i.id LIMIT {LIMIT} MATCH (i) -[:IN_MOSAIC]-> (m:Mosaic) RETURN i, m"""
        ...
        # Key Frame Stuff
        ###############################
        query8 = f"""{images_and_inspections} 
                    WITH i OPTIONAL MATCH (i) -[:IN_MOSAIC]-> (m)
                    WITH coalesce(m.image_file, i.thumbnail) as image_file, coalesce(m.id, i.id) as id, coalesce(m.uciqe, i.uciqe) as uciqe
                    RETURN DISTINCT image_file, uciqe"""
        if not filter_options.mosaics:
        ...
        with neo4j_transaction() as tx:
            ...
            results = [(r['i'], r['i2'], r['d']) for r in cursor]
            for image1, image2, distance in results:
                image1_id = f"im_{image1['id']}"
                image2_id = f"im_{image2['id']}"
                #nodes[image2_id] = dict(image2.nodes[0])
            ...
            cursor = tx.run(query8)
            image_table = [{'image':f"![{r['image_file']}](assets/imgs/{'mosaics' if r['image_file'].startswith('m') else 'frames'}/{r['image_file']})", 'quality': r['uciqe']} for r in cursor]
        ...
        similar_to = list(similar_to)
        visually_similar_to = list(visually_similar_to)
        in_inspection = list(in_inspection)
        in_mosaic = list(in_mosaic)
        in_cluster = list(in_cluster)
        shows_part = list(shows_part)
        part_of_ship = list(part_of_ship)
        data = {
            'nodes': nodes,
            ...
            'shows_part': shows_part,
            'part_of_ship': part_of_ship,
            'filter_id': filter_options.filter_id,
            'q_hist': q_hist,
            'd_hist': d_hist,
            'mg_hist': mg_hist,
            'sim_hist': sim_hist,
            'vsim_hist': vsim_hist,
            'heading_hist': heading_hist,
            'by_ship_table': table_ships,
            'by_part_table': table_parts,
            'image_table': image_table,
            'clusters_table': table_clusters,
        }
        print(f"data sucessfully retrieved, it has {len(pickle.dumps(data)):,} bytes, storing data in data storage...")
        return data