...
class Mosaic:
    ...
    def process_frame(self, frame_cur):
        ...
        print(str(i-START_FRAME)+": "+str(distance))   
        if distance > 100: 
...
class LIACI_Stitcher():
    def __init__(self):
        self.segmenter = LIACi_segmenter.LIACi_segmenter()
        self.is_first_frame = True
    
        self.failed_number_of_frames = 0
        self.starting_point = np.array([0, 0])

    def get_current_mosaic(self):
        return self.video
    ...
    def next_frame(self, frame):
        ...
        if self.is_first_frame:
            ...
            self.failed_number_of_frames = 0         
            self.seg_output_img = np.zeros((video_mosaic.output_img.shape[0],video_mosaic.output_img.shape[1],3), np.uint8)
            self.failed_number_of_frames = 0

        # process each frame
        ret_val = video_mosaic.process_frame(frame_cur)

        if self.failed_number_of_frames > 2:
        ...
        if not ret_val:
            self.failed_number_of_frames += 1
            return False
                  
        self.failed_number_of_frames += 1

        frame_cur_seg_mg = np.zeros((frame_cur.shape[0],frame_cur.shape[1]), np.uint8) # marine growth
        frame_cur_seg_sh = np.zeros((frame_cur.shape[0],frame_cur.shape[1]), np.uint8) # ship hull
        frame_cur_seg_pp = np.zeros((frame_cur.shape[0],frame_cur.shape[1]), np.uint8) # paint peel
        ...
        for value in seg_results:      
            ...
            if value['tagName'] == 'paint_peel':
                mask_pp = np.reshape(value['mask']['mask_array'], (value['mask']['height'],value['mask']['width'])).astype(np.uint8)
                frame_cur_seg_pp = cv2.bitwise_or(frame_cur_seg_pp, mask_pp)          
        ...
        seg_output_mask1 = cv2.bitwise_or(seg_output_mask1, warped_seg_img_sh)
        seg_output_img[seg_output_mask1 > 0] = np.flip(segmenter.COLORS[9], 0)
        ...
        seg_output_mask2 = cv2.bitwise_or(seg_output_mask2, warped_seg_img_mg)
        seg_output_img[seg_output_mask2 > 0] = np.flip(segmenter.COLORS[4], 0)
        warped_seg_img_pp = cv2.warpPerspective(
                frame_cur_seg_pp, video_mosaic.H, (video_mosaic.output_img.shape[1], video_mosaic.output_img.shape[0]), flags=cv2.INTER_CUBIC)
        seg_output_mask3 = cv2.bitwise_or(seg_output_mask3, warped_seg_img_pp)
        seg_output_img[seg_output_mask3 > 0] = np.flip(segmenter.COLORS[6], 0)
...
if __name__ == "__main__":
    ...
    START_FRAME = int(video_capture.get(cv2.CAP_PROP_FPS)*(35*60+49))+335
    NUM_OF_FRAMES = 500
    ...
    for i in range(START_FRAME,START_FRAME+NUM_OF_FRAMES,FRAME_STEP):
        ...
        seg_results = segmenter.segment_unet()
        if is_first_frame:
            video_mosaic = LIACi_stitcher(frame_cur, detector_type="sift")           
