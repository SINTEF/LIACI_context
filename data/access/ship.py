from data.access.datastore import EntryDoesExistExeption, find_node, liaci_graph, neo4j_transaction
...
def create(ship: LiShip, fail_on_exists = False):
    ...
    with neo4j_transaction() as tx:
        ...
        node_01 = py2neo.Node(obj.name, "Classification", name=obj.name, visCode=obj.visCode)
        tx.create(node_01)   
        tx.create(py2neo.Relationship(node_ship, "HAS", node_01))
        
        obj = ship.LiPropellerBladeSealingTightness
        node_02 = py2neo.Node(obj.name, "Classification", name=obj.name, visCode=obj.visCode)
        tx.create(node_02)   
        ...
        node_03 = py2neo.Node(obj.name, "Classification", name=obj.name, visCode=obj.visCode)
        tx.create(node_03)   
        ...
        node_04 = py2neo.Node(obj.name, "Classification", name=obj.name, visCode=obj.visCode)
        tx.create(node_04)   
        tx.create(py2neo.Relationship(node_01, "HAS", node_04))

        obj = ship.LiFlapBeckerRudder
        node_05 = py2neo.Node(obj.name, "Classification", name=obj.name, visCode=obj.visCode)
        tx.create(node_05)   
